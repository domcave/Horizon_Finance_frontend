...
function Dashboard() {
  ...
  return (
    <div className="dashboard-page">
      ...
      <button
        onClick={() => {
          setTransactionsMonth(
            PlaidService.getTransactionsThisMonth("user1").json()
          );
        }}
      >
      ...
      <button
        onClick={() => {
          RecommendationService.getHouseRecommendation({
            username: "user1",
            owned_house_price: 250000,
          });
        }}
      >
        Click to get new house recommendation
      </button>
      <button
        onClick={() => {
          RecommendationService.getMarriageRecommendation({
            username: "user1",
            num_kids: 2,
            arr: 0.08,
            spouse_income: 65000,
      ...
      >
        Click to get marraige recommendation
      </button>
      <button
        onClick={() => {
          RecommendationService.getRetirementRecommendation({
            username: "user1",
            savings: 300000,
            arr: 0.07,
            wd_rate: 0.04,
      ...
      >
        Click to get retirement recommendation
      </button>
    </div>
  );
}
